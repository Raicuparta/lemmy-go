...
async function getUrlFromText(text) {
    ...
    return `${getPreferredInstanceUrl()}/search?q=${encodeURIComponent(text)}&type=Communities`;
}
function getPreferredInstanceUrl() {
    const instanceDomain = (storage?.instanceDomain || fallbackInstanceDomain).trim();
    return `https://${instanceDomain.endsWith("/") ? instanceDomain.slice(0, -1) : instanceDomain}`;
}
...
async function setUpCommunities() {
    ...
    communities = communitiesJson.sort((communityA, communityB) => communityB.subscribers - communityA.subscribers);
}
const formatCommunity = (community) => `${escapeOmniboxString(community.title)} (${getCommunityId(community)}, ${community.subscribers} subs)`;
...
async function getFilteredCommunities(text) {
    ...
    score(community, text) >= 0 &&
        ...
        .sort((communityA, communityB) => score(communityA, text) - score(communityB, text))
        .slice(0, 10));
}
...
function score(community, query) {
    ...
    if (id === normalizedQuery)
        return 0;
    if (normalizedQuery.includes("@") && id.includes(normalizedQuery))
        return 1;
...
chrome.omnibox.onInputChanged.addListener(async (text, suggest) => {
    ...
    suggest(filteredCommunities.map((community) => ({
